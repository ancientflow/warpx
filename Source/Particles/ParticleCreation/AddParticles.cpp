...
applyBallisticCorrection (const XDim3& pos, const InjectorMomentum* inj_mom,
                          amrex::Real gamma_boost, amrex::Real beta_boost,
                          amrex::Real t) noexcept {
    const XDim3 u_bulk = inj_mom->getBulkMomentum(pos.x, pos.y, pos.z);
    const amrex::Real gamma_bulk =
        std::sqrt(1._rt + (u_bulk.x * u_bulk.x + u_bulk.y * u_bulk.y +
                           u_bulk.z * u_bulk.z));
    const amrex::Real betaz_bulk = u_bulk.z / gamma_bulk;
    const amrex::Real z0 =
        gamma_boost * (pos.z * (1.0_rt - beta_boost * betaz_bulk) -
                       PhysConst::c * t * (betaz_bulk - beta_boost));
    return z0;
}

AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE XDim3
getCellCoords (const amrex::GpuArray<Real, AMREX_SPACEDIM>& lo_corner,
               const amrex::GpuArray<Real, AMREX_SPACEDIM>& dx, const XDim3& r,
               const amrex::IntVect& iv) noexcept {
    XDim3 pos;
...
/**
 * \brief This function is called in AddPlasma when we want a particle to be
 * removed at the next call to redistribute. It initializes all the particle
 * properties to zero (to be safe and avoid any possible undefined behavior
 * before the next call to redistribute) and sets the particle id to -1 so that
 * it can be effectively deleted.
 *
...
ZeroInitializeAndSetNegativeID (
    ...
    if (qed_helper.has_quantum_sync) {
        qed_helper.p_optical_depth_QSR[ip] = 0._rt;
    }
    if (qed_helper.has_breit_wheeler) {
        qed_helper.p_optical_depth_BW[ip] = 0._rt;
    }
#endif
...
PhysicalParticleContainer::AddParticles (int lev) {
    ...
    for (auto const& plasma_injector : plasma_injectors) {
        ...
        if (plasma_injector->add_single_particle) {
            ...
            const amrex::Vector<ParticleReal> xp = {
                plasma_injector->single_particle_pos[0]};
            const amrex::Vector<ParticleReal> yp = {
                plasma_injector->single_particle_pos[1]};
            const amrex::Vector<ParticleReal> zp = {
                plasma_injector->single_particle_pos[2]};
            const amrex::Vector<ParticleReal> uxp = {
                plasma_injector->single_particle_u[0]};
            const amrex::Vector<ParticleReal> uyp = {
                plasma_injector->single_particle_u[1]};
            const amrex::Vector<ParticleReal> uzp = {
                plasma_injector->single_particle_u[2]};
            const amrex::Vector<amrex::Vector<ParticleReal>> attr = {
                {plasma_injector->single_particle_weight}};
            const amrex::Vector<amrex::Vector<int>> attr_int;
            AddNParticles(lev, 1, xp, yp, zp, uxp, uyp, uzp, 1, attr, 0,
                          attr_int, 0);
            return;
        }
        ...
        if (plasma_injector->add_multiple_particles) {
            if (WarpX::gamma_boost > 1.) {
                for (int i = 0;
                     i < plasma_injector->multiple_particles_pos_x.size();
                     i++) {
                    MapParticletoBoostedFrame(
                        plasma_injector->multiple_particles_pos_x[i],
            ...
            AddNParticles(lev,
                          ...
                          plasma_injector->multiple_particles_uz, 1, attr, 0,
                          attr_int, 0);
        }
        ...
        if (plasma_injector->external_file) {
            AddPlasmaFromFile(*plasma_injector, plasma_injector->q_tot,
 ...
 */
void
PhysicalParticleContainer::ContinuousInjection (
    const amrex::RealBox& injection_box) {
 ...
 */
void
PhysicalParticleContainer::ContinuousFluxInjection (amrex::Real t,
                                                    amrex::Real dt) {
    for (auto const& plasma_injector : plasma_injectors) {
        if (plasma_injector->doFluxInjection()) {
            // Check the optional parameters for start and stop of injection
            if (((plasma_injector->flux_tmin < 0) ||
                 (t >= plasma_injector->flux_tmin)) &&
                ((plasma_injector->flux_tmax < 0) ||
                 (t < plasma_injector->flux_tmax))) {
...
void
PhysicalParticleContainer::MapParticletoBoostedFrame (
    ...
    const amrex::ParticleReal uz_boost =
        WarpX::gamma_boost * WarpX::beta_boost * PhysConst::c;

    // tpr is the particle's time in the boosted frame
    const amrex::ParticleReal tpr =
        WarpX::gamma_boost * t_lab -
        uz_boost * z / (PhysConst::c * PhysConst::c);
    ...
    const amrex::ParticleReal gamma_lab = std::sqrt(
        1._rt + (ux * ux + uy * uy + uz * uz) / (PhysConst::c * PhysConst::c));
    // ux = ux;
    // uy = uy;
    uz = WarpX::gamma_boost * uz - uz_boost * gamma_lab;
    const amrex::ParticleReal gammapr = std::sqrt(
        1._rt + (ux * ux + uy * uy + uz * uz) / (PhysConst::c * PhysConst::c));
    ...
    // Move the particles to where they will be at t = t0, the current
    // simulation time in the boosted frame
    constexpr int lev = 0;
...
PhysicalParticleContainer::CheckAndAddParticle (
    ...
    amrex::Gpu::HostVector<ParticleReal>& particle_w, amrex::Real t_lab) const {
    if (WarpX::gamma_boost > 1.) {
...
PhysicalParticleContainer::AddGaussianBeam (
    ...
    const amrex::Vector<amrex::Real> rotation_axis =
        plasma_injector.rotation_axis;
        ...
        for (long i = 0; i < npart; ++i) {
            ...
            if (plasma_injector.insideBounds(x, y, z) &&
                ...
                if (plasma_injector.do_focusing) {
                    const XDim3 u_bulk =
                        plasma_injector.getInjectorMomentumHost()
                            ->getBulkMomentum(x, y, z);
                    const amrex::Real u_bulk_norm =
                        std::sqrt(u_bulk.x * u_bulk.x + u_bulk.y * u_bulk.y +
                                  u_bulk.z * u_bulk.z);

                    // Compute the position of the focal plane
                    // (it is located at a distance `focal_distance` from the
                    // beam centroid, in the direction of the bulk velocity)
                    ...
                    const amrex::Real gamma =
                        std::sqrt(1._rt + (u.x * u.x + u.y * u.y + u.z * u.z));
                    ...
                    // Compute the time at which the particle will cross the
                    // focal plane
                    const amrex::Real v_dot_n =
                        v_x * n_x + v_y * n_y + v_z * n_z;
                    const amrex::Real t =
                        ((x_f - x) * n_x + (y_f - y) * n_y + (z_f - z) * n_z) /
                        v_dot_n;

                    // Displace particles in the direction orthogonal to the
                    // beam bulk momentum i.e. orthogonal to (n_x, n_y, n_z)
#if defined(WARPX_DIM_3D) || defined(WARPX_DIM_RZ)
                ...
                if (plasma_injector.do_rotation) {
                    ...
                    const Real k_norm =
                        std::sqrt(rotation_axis[0] * rotation_axis[0] +
...
#if defined(WARPX_DIM_3D)
                    x = x_m + (x - x_m) * std::cos(rotation_angle) +
                        k_cross_x * std::sin(rotation_angle) +
                        kx * k_dot_x * (1._rt - std::cos(rotation_angle));
                    y = y_m + (y - y_m) * std::cos(rotation_angle) +
                        k_cross_y * std::sin(rotation_angle) +
                        ky * k_dot_x * (1._rt - std::cos(rotation_angle));
                    z = z_m + (z - z_m) * std::cos(rotation_angle) +
                        k_cross_z * std::sin(rotation_angle) +
                        kz * k_dot_x * (1._rt - std::cos(rotation_angle));
#elif defined(WARPX_DIM_XZ)
                    x = x_m + (x - x_m) * std::cos(rotation_angle) +
                        k_cross_x * std::sin(rotation_angle) +
                        kx * k_dot_x * (1._rt - std::cos(rotation_angle));
                    ...
                    if (plasma_injector.do_rotation_momenta) {
                        ...
                        // rotate momenta
                        u.x = u.x * std::cos(rotation_angle) +
                              k_cross_u_x * std::sin(rotation_angle) +
                              kx * k_dot_u * (1._rt - std::cos(rotation_angle));
                        u.y = u.y * std::cos(rotation_angle) +
                              k_cross_u_y * std::sin(rotation_angle) +
                              ky * k_dot_u * (1._rt - std::cos(rotation_angle));
                        u.z = u.z * std::cos(rotation_angle) +
                              k_cross_u_z * std::sin(rotation_angle) +
                              kz * k_dot_u * (1._rt - std::cos(rotation_angle));
                    }
                }
#else
                ...
                if (do_symmetrize && symmetrization_order == 8) {
                    ...
                    CheckAndAddParticle(x, y, z, u.x, u.y, u.z, weight / 8._rt,
                                        ...
                                        particle_w);
                    CheckAndAddParticle(x, -y, z, u.x, -u.y, u.z,
                                        weight / 8._rt, particle_x, particle_y,
                                        particle_z, particle_ux, particle_uy,
                                        particle_uz, particle_w);
                    CheckAndAddParticle(-x, y, z, -u.x, u.y, u.z,
                                        weight / 8._rt, particle_x, particle_y,
                                        particle_z, particle_ux, particle_uy,
                                        particle_uz, particle_w);
                    ...
                    CheckAndAddParticle(y, x, z, u.y, u.x, u.z, weight / 8._rt,
                                        ...
                                        particle_ux, particle_uy, particle_uz,
                                        particle_w);
                    CheckAndAddParticle(-y, x, z, -u.y, u.x, u.z,
                                        weight / 8._rt, particle_x, particle_y,
                                        particle_z, particle_ux, particle_uy,
                                        particle_uz, particle_w);
                    CheckAndAddParticle(y, -x, z, u.y, -u.x, u.z,
                    ...
                    CheckAndAddParticle(x, y, z, u.x, u.y, u.z, weight / 4._rt,
                                        ...
                                        particle_w);
                    CheckAndAddParticle(x, -y, z, u.x, -u.y, u.z,
                                        weight / 4._rt, particle_x, particle_y,
                                        particle_z, particle_ux, particle_uy,
                                        particle_uz, particle_w);
                    CheckAndAddParticle(-x, y, z, -u.x, u.y, u.z,
                                        weight / 4._rt, particle_x, particle_y,
    ...
    const amrex::Vector<ParticleReal> xp(particle_x.data(),
                                         particle_x.data() + np);
    const amrex::Vector<ParticleReal> yp(particle_y.data(),
                                         particle_y.data() + np);
    const amrex::Vector<ParticleReal> zp(particle_z.data(),
                                         particle_z.data() + np);
    const amrex::Vector<ParticleReal> uxp(particle_ux.data(),
                                          particle_ux.data() + np);
    const amrex::Vector<ParticleReal> uyp(particle_uy.data(),
                                          particle_uy.data() + np);
    const amrex::Vector<ParticleReal> uzp(particle_uz.data(),
                                          particle_uz.data() + np);

    amrex::Vector<amrex::Vector<ParticleReal>> attr;
    const amrex::Vector<ParticleReal> wp(particle_w.data(),
                                         particle_w.data() + np);
    attr.push_back(wp);
...
PhysicalParticleContainer::AddPlasmaFromFile (PlasmaInjector& plasma_injector,
    ...
    if (ParallelDescriptor::IOProcessor()) {
        ...
        auto series = std::any_cast<openPMD::Series>(
            std::move(plasma_injector.m_openpmd_input_series));
        ...
        const std::shared_ptr<ParticleReal> ptr_x =
            ps["position"]["x"].loadChunk<ParticleReal>();
        const std::shared_ptr<ParticleReal> ptr_offset_x =
            ps["positionOffset"]["x"].loadChunk<ParticleReal>();
        auto const position_unit_x =
            static_cast<ParticleReal>(ps["position"]["x"].unitSI());
        auto const position_offset_unit_x =
            static_cast<ParticleReal>(ps["positionOffset"]["x"].unitSI());
#endif
#if !(defined(WARPX_DIM_XZ) || defined(WARPX_DIM_1D_Z))
        const std::shared_ptr<ParticleReal> ptr_y =
            ps["position"]["y"].loadChunk<ParticleReal>();
        const std::shared_ptr<ParticleReal> ptr_offset_y =
            ps["positionOffset"]["y"].loadChunk<ParticleReal>();
        auto const position_unit_y =
            static_cast<ParticleReal>(ps["position"]["y"].unitSI());
        auto const position_offset_unit_y =
            static_cast<ParticleReal>(ps["positionOffset"]["y"].unitSI());
#endif
#if !defined(WARPX_DIM_RCYLINDER)
        const std::shared_ptr<ParticleReal> ptr_z =
            ps["position"]["z"].loadChunk<ParticleReal>();
        const std::shared_ptr<ParticleReal> ptr_offset_z =
            ps["positionOffset"]["z"].loadChunk<ParticleReal>();
        auto const position_unit_z =
            static_cast<ParticleReal>(ps["position"]["z"].unitSI());
        auto const position_offset_unit_z =
            static_cast<ParticleReal>(ps["positionOffset"]["z"].unitSI());
#endif

        const std::shared_ptr<ParticleReal> ptr_ux =
            ps["momentum"]["x"].loadChunk<ParticleReal>();
        auto const momentum_unit_x =
            static_cast<ParticleReal>(ps["momentum"]["x"].unitSI());
        const std::shared_ptr<ParticleReal> ptr_uz =
            ps["momentum"]["z"].loadChunk<ParticleReal>();
        auto const momentum_unit_z =
            static_cast<ParticleReal>(ps["momentum"]["z"].unitSI());
        const std::shared_ptr<ParticleReal> ptr_w =
            ps["weighting"][openPMD::RecordComponent::SCALAR]
                .loadChunk<ParticleReal>();
        ...
        if (ps["momentum"].contains("y")) {
            ...
            momentum_unit_y =
                static_cast<ParticleReal>(ps["momentum"]["y"].unitSI());
        }
        series.flush(); // shared_ptr data can be read now

        if (q_tot != 0.0) {
            std::stringstream warnMsg;
            warnMsg << " Loading particle species from file. " << ps_name
                    << ".q_tot is ignored.";
            ablastr::warn_manager::WMRecordWarning(
                "AddPlasmaFromFile", warnMsg.str(),
    ...
    defined(WARPX_DIM_RCYLINDER) || defined(WARPX_DIM_RSPHERE)
            ...
            amrex::ParticleReal const z =
                ptr_z.get()[i] * position_unit_z +
                ptr_offset_z.get()[i] * position_offset_unit_z + z_shift;
#else
            ...
            if (plasma_injector.insideBounds(x, y, z)) {
                ...
                amrex::ParticleReal const mass_eff =
                    (m_mass > 0.0_prt) ? m_mass : PhysConst::m_e;
                amrex::ParticleReal const ux =
                    ptr_ux.get()[i] * momentum_unit_x / mass_eff;
                amrex::ParticleReal const uz =
                    ptr_uz.get()[i] * momentum_unit_z / mass_eff;
                amrex::ParticleReal uy = 0.0_prt;
                if (ps["momentum"].contains("y")) {
                    uy = ptr_uy.get()[i] * momentum_unit_y / mass_eff;
                }
                CheckAndAddParticle(x, y, z, ux, uy, uz, weight, particle_x,
                                    particle_y, particle_z, particle_ux,
                                    particle_uy, particle_uz, particle_w,
                                    static_cast<amrex::Real>(t_lab));
            }
        }
        auto const np = particle_z.size();
        if (np < npart) {
            ablastr::warn_manager::WMRecordWarning(
                "Species", "Simulation box doesn't cover all particles",
                ablastr::warn_manager::WarnPriority::high);
        }
    } // IO Processor
    auto const np = static_cast<long>(particle_z.size());
...
PhysicalParticleContainer::AddPlasma (PlasmaInjector& plasma_injector, int lev,
    ...
    const bool refine_injection =
        findRefinedInjectionBox(fine_injection_box, rrfac);
...
#if defined(WARPX_DIM_RZ) || defined(WARPX_DIM_RCYLINDER) ||                   \
    defined(WARPX_DIM_RSPHERE)
    const amrex::Real radial_numpercell_power =
        plasma_injector.radial_numpercell_power;
#endif
    ...
    const PlasmaParserWrapper plasma_parser_wrapper(
        m_user_int_attribs.size(), m_user_real_attribs.size(),
        m_user_int_attrib_parser, m_user_real_attrib_parser);

    auto get_zlab = [=] (amrex::Real z) -> amrex::Real {
        return applyBallisticCorrection(amrex::XDim3{0._rt, 0._rt, z},
                                        h_inj_mom, gamma_boost, beta_boost, t);
    };
    ...
    if (initial_injection) {
        ...
        plasma_injector.prepare(part_realbox, moving_dir, moving_sign,
                                get_zlab);
    }
...
#pragma omp parallel if (not WarpX::serialize_initial_conditions &&            \
    ...
    for (MFIter mfi = MakeMFIter(lev, info); mfi.isValid(); ++mfi) {
        ...
        const bool no_overlap =
            find_overlap(tile_realbox, part_realbox, dx, problo,
                         overlap_realbox, overlap_box, shifted);
        if (no_overlap) {
        ...
        const amrex::GpuArray<amrex::Real, AMREX_SPACEDIM> overlap_corner{
            AMREX_D_DECL(overlap_realbox.lo(0), overlap_realbox.lo(1),
        ...
        if (refine_injection) {
            fine_overlap_box =
                overlap_box & amrex::shift(fine_injection_box, -shifted);
        }
        amrex::ParallelFor(overlap_box, [=] AMREX_GPU_DEVICE(int i, int j,
                                                             int k) noexcept {
            const amrex::IntVect iv(AMREX_D_DECL(i, j, k));
            auto lo =
                getCellCoords(overlap_corner, dx, {0._rt, 0._rt, 0._rt}, iv);
            auto hi =
                getCellCoords(overlap_corner, dx, {1._rt, 1._rt, 1._rt}, iv);

            lo.z = applyBallisticCorrection(lo, inj_mom, gamma_boost,
                                            beta_boost, t);
            hi.z = applyBallisticCorrection(hi, inj_mom, gamma_boost,
                                            beta_boost, t);

            if (inj_pos->overlapsWith(lo, hi)) {
                auto index = overlap_box.index(iv);
                const amrex::Long r =
                    (fine_overlap_box.ok() && fine_overlap_box.contains(iv))
                        ? (AMREX_D_TERM(rrfac[0], *rrfac[1], *rrfac[2]))
                        : (1);
                pcounts[index] = num_ppc * r;
                // update pcount by checking if cell-corners or cell-center
                // has non-zero density
                const auto xlim =
                    amrex::GpuArray<Real, 3>{lo.x, (lo.x + hi.x) / 2._rt, hi.x};
                const auto ylim =
                    amrex::GpuArray<Real, 3>{lo.y, (lo.y + hi.y) / 2._rt, hi.y};
                const auto zlim =
                    amrex::GpuArray<Real, 3>{lo.z, (lo.z + hi.z) / 2._rt, hi.z};
...
#pragma omp critical(add_plasma_nextid)
        ...
        uint64_t* AMREX_RESTRICT pa_idcpu =
            soa.GetIdCPUData().data() + old_size;

        PlasmaParserHelper plasma_parser_helper(
            soa, old_size, m_user_int_attribs, m_user_real_attribs,
            plasma_parser_wrapper);
        int** pa_user_int_data = plasma_parser_helper.getUserIntDataPtrs();
        amrex::ParticleReal** pa_user_real_data =
            plasma_parser_helper.getUserRealDataPtrs();
        amrex::ParserExecutor<7> const* user_int_parserexec_data =
            plasma_parser_helper.getUserIntParserExecData();
        amrex::ParserExecutor<7> const* user_real_parserexec_data =
            plasma_parser_helper.getUserRealParserExecData();
...
#ifdef WARPX_QED
        const QEDHelper qed_helper(soa, old_size, has_quantum_sync(),
                                   has_breit_wheeler(), m_shr_p_qs_engine,
                                   m_shr_p_bw_engine);
#endif
...
#endif
        amrex::ParallelForRNG(overlap_box, [=] AMREX_GPU_DEVICE(
                                               int i, int j, int k,
            ...
            if (rz_random_theta) {
                theta_offset = amrex::Random(engine) * 2._rt * MathConst::pi;
            }
#endif

            const amrex::Real scale_fac =
                compute_scale_fac_volume(dx, pcounts[index]);
            for (int i_part = 0; i_part < pcounts[index]; ++i_part) {
                ...
                const XDim3 r =
                    (fine_overlap_box.ok() && fine_overlap_box.contains(iv))
                        ?
                        // In the refined injection region: use refinement ratio
                        // `rrfac`
                        inj_pos->getPositionUnitBox(i_part, rrfac, engine)
                        :
                        // Otherwise: use 1 as the refinement ratio
                        inj_pos->getPositionUnitBox(
                            i_part, amrex::IntVect::TheUnitVector(), engine);
                auto pos = getCellCoords(overlap_corner, dx, r, iv);

#if defined(WARPX_DIM_3D)
                bool const box_contains =
                    tile_realbox.contains(XDim3{pos.x, pos.y, pos.z});
...
#endif
                if (!box_contains) {
                    ZeroInitializeAndSetNegativeID(pa_idcpu, pa, ip,
                                                   loc_do_field_ionization, pi
#ifdef WARPX_QED
                                                   ,
                                                   qed_helper
#endif
                ...
                const amrex::Real theta =
                    (nmodes == 1 && rz_random_theta)
                        ?
#if defined(WARPX_DIM_RZ)
                        // This should be updated to be the same as below, since
                        // theta should range from -pi to +pi. This should be a
                        // separate PR since it will break RZ CI tests.
                ...
                // Adjust the particle radius to produce the correct
                // distribution. Note that this may shift particles outside of
                // the current tile, but this is Ok since particles will be
                // redistributed afterwards. The tile_realbox.contains check
                // above ensures that the "logical" space is uniformly filled.
                amrex::Real const xu = (pos.x - rmin) / (rmax - rmin);
                amrex::Real const rc =
                    std::pow(rmax, 1._rt + radial_numpercell_power) -
                    std::pow(rmin, 1._rt + radial_numpercell_power);
                amrex::Real const rminp =
                    std::pow(rmin, 1._rt + radial_numpercell_power);
                amrex::Real const xb = std::pow(
                    xu * rc + rminp, 1._rt / (1._rt + radial_numpercell_power));
                ...
                if (gamma_boost == 1._rt) {
                    ...
                    // include ballistic correction for plasma species with bulk
                    // motion
                    const amrex::Real z0 = applyBallisticCorrection(
                        pos, inj_mom, gamma_boost, beta_boost, t);
                    if (!inj_pos->insideBounds(xb, yb, z0)) {
                        ZeroInitializeAndSetNegativeID(
                            pa_idcpu, pa, ip, loc_do_field_ionization, pi
#ifdef WARPX_QED
                            ,
                            qed_helper
#endif
                    ...
                    const amrex::Real z0_lab = applyBallisticCorrection(
                        pos, inj_mom, gamma_boost, beta_boost, t);

                    // If the particle is not within the lab-frame zmin, zmax,
                    // etc. go to the next generated particle.
                    ...
                    dens =
                        gamma_boost * dens * (1.0_rt - beta_boost * betaz_lab);
...
#ifdef WARPX_QED
                if (qed_helper.has_quantum_sync) {
                    qed_helper.p_optical_depth_QSR[ip] =
                        qed_helper.quantum_sync_get_opt(engine);
                }

                if (qed_helper.has_breit_wheeler) {
                    qed_helper.p_optical_depth_BW[ip] =
                        qed_helper.breit_wheeler_get_opt(engine);
                }
#endif
                // Initialize user-defined integers with user-defined parser
                for (int ia = 0; ia < n_user_int_attribs; ++ia) {
                    pa_user_int_data[ia][ip] =
                        static_cast<int>(user_int_parserexec_data[ia](
                            pos.x, pos.y, pos.z, u.x, u.y, u.z, t));
                }
                // Initialize user-defined real attributes with user-defined
                // parser
                for (int ia = 0; ia < n_user_real_attribs; ++ia) {
                    pa_user_real_data[ia][ip] = user_real_parserexec_data[ia](
                        pos.x, pos.y, pos.z, u.x, u.y, u.z, t);
                }
    ...
    if (EB::enabled()) {
        ...
        scrapeParticlesAtEB(*this,
                            warpx.m_fields.get_mr_levels(
                                FieldType::distance_to_eb, warpx.finestLevel()),
...
PhysicalParticleContainer::AddPlasmaFlux (PlasmaInjector const& plasma_injector,
    ...
    const amrex::Real num_ppc_real =
        plasma_injector.num_particles_per_cell_real;
#if defined(WARPX_DIM_RZ) || defined(WARPX_DIM_RCYLINDER) ||                   \
    defined(WARPX_DIM_RSPHERE)
    const amrex::Real rmax =
        std::min(plasma_injector.xmax, geom.ProbDomain().hi(0));
    const amrex::Real rmin =
        std::max(plasma_injector.xmin, geom.ProbDomain().lo(0));
#endif
...
#ifdef AMREX_USE_EB
    bool const inject_from_eb =
        plasma_injector
            .m_inject_from_eb; // whether to inject from EB or from a plane
    ...
    for (int ic = 0; ic < NumRuntimeRealComps(); ++ic) {
        tmp_pc.AddRealComp(GetRealSoANames()[ic + NArrayReal], false);
    }
    for (int ic = 0; ic < NumRuntimeIntComps(); ++ic) {
        tmp_pc.AddIntComp(GetIntSoANames()[ic + NArrayInt], false);
    }
...
#pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi = MakeMFIter(0, info); mfi.isValid(); ++mfi) {
...
#endif
        {
            ...
            const bool no_overlap = find_overlap_flux(
                tile_realbox, part_realbox, dx, problo, plasma_injector,
                overlap_realbox, overlap_box, shifted);
            if (no_overlap) {
                continue;
            } // Go to the next tile
...
#ifdef AMREX_USE_EB
        auto eb_flag_arr =
            eb_flag ? eb_flag->const_array(mfi) : Array4<EBCellFlag const>{};
        ...
        amrex::ParallelForRNG(overlap_box, [=] AMREX_GPU_DEVICE(
            ...
            amrex::Real num_ppc_real_in_this_cell =
                num_ppc_real; // user input: number of macroparticles per cell
            if (inject_from_eb) {
                // Injection from EB
                ...
                num_ppc_real_in_this_cell *=
                    eb_data.get<amrex::EBData_t::bndryarea>(i, j, k);
            }
#else
            amrex::Real const num_ppc_real_in_this_cell = num_ppc_real; // user input: number of macroparticles per cell
#endif
            // Skip cells that do not overlap with the bounds specified by the
            // user (xmin/xmax, ymin/ymax, zmin/zmax)
            auto lo =
                getCellCoords(overlap_corner, dx, {0._rt, 0._rt, 0._rt}, iv);
            auto hi =
                getCellCoords(overlap_corner, dx, {1._rt, 1._rt, 1._rt}, iv);
            ...
            const int num_ppc_int = static_cast<int>(
                num_ppc_real_in_this_cell * r + amrex::Random(engine));
...
#pragma omp critical(add_plasma_nextid)
        ...
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(pid + max_new_particles <
                                             LongParticleIds::LastParticleID,
                                         "overflow on particle id numbers");

        const int cpuid = ParallelDescriptor::MyProc();

        auto& particle_tile =
            tmp_pc.DefineAndReturnParticleTile(0, grid_id, tile_id);
    ...
    defined(WARPX_DIM_RSPHERE)
        ...
        amrex::ParallelForRNG(overlap_box, [=] AMREX_GPU_DEVICE(
...
#ifdef AMREX_USE_EB
            if (inject_from_eb) {
                scale_fac = compute_scale_fac_area_eb(
                    dx, num_ppc_real,
                    AMREX_D_DECL(
                        eb_data.get<amrex::EBData_t::bndrynorm>(i, j, k, 0),
                        eb_data.get<amrex::EBData_t::bndrynorm>(i, j, k, 1),
                        eb_data.get<amrex::EBData_t::bndrynorm>(i, j, k, 2)));
            } else
#endif
            {
                scale_fac = compute_scale_fac_area_plane(dx, num_ppc_real,
                                                         flux_normal_axis);
            }
            ...
            for (int i_part = 0; i_part < pcounts[index]; ++i_part) {
...
#if defined(WARPX_DIM_3D)
                    pos.x =
                        overlap_corner[0] + (iv[0] + 0.5_rt + pt[0]) * dx[0];
                    pos.y =
                        overlap_corner[1] + (iv[1] + 0.5_rt + pt[1]) * dx[1];
                    pos.z =
                        overlap_corner[2] + (iv[2] + 0.5_rt + pt[2]) * dx[2];
#elif defined(WARPX_DIM_XZ) || defined(WARPX_DIM_RZ)
                ...
                {
                    // Injection from a plane
                    // This assumes the flux_pos is of type
                    // InjectorPositionRandomPlane
                    r = (fine_overlap_box.ok() && fine_overlap_box.contains(iv))
                            ?
                            // In the refined injection region: use refinement
                            // ratio `rrfac`
                            flux_pos->getPositionUnitBox(i_part, rrfac, engine)
                            :
                            // Otherwise: use 1 as the refinement ratio
                            flux_pos->getPositionUnitBox(
                                i_part, amrex::IntVect::TheUnitVector(),
                                engine);
                ...
                // The containsInclusive is used to allow the case of the flux
                // surface being on the boundary of the domain. After the
                // UpdatePosition below, the particles will be within the
                // domain.
#if defined(WARPX_DIM_3D)
                if (!ParticleUtils::containsInclusive(
                        tile_realbox, XDim3{ppos.x, ppos.y, ppos.z})) {
...
#endif
                // Lab-frame simulation
                // If the particle's initial position is not within or on the
                // species's xmin, xmax, ymin, ymax, zmin, zmax, go to the next
                // generated particle.
...
#ifdef AMREX_USE_EB
                if (inject_from_eb) {
                    // Injection from EB: rotate momentum according to the
                    // normal of the EB surface (The above code initialized the
                    // momentum by assuming that z is the direction normal to
                    // the EB surface. Thus we need to rotate from z to the
                    // normal.)
                    rotate_momentum_eb(
                        pu,
                        AMREX_D_DECL(
                            eb_data.get<amrex::EBData_t::bndrynorm>(i, j, k, 0),
                            eb_data.get<amrex::EBData_t::bndrynorm>(i, j, k, 1),
                            eb_data.get<amrex::EBData_t::bndrynorm>(i, j, k,
                                                                    2)));
                }
#endif
                ...
                // distribution. Note that this may shift particles outside of
                // the current tile, but this is Ok since particles will be
                // redistributed afterwards. The containsInclusive check above
                // ensures that the "logical" space is uniformly filled.
                ...
                amrex::Real const rminp =
                    std::pow(rmin, 1._rt + radial_numpercell_power);
                amrex::Real const radial_position = std::pow(
                    xu * rc + rminp, 1._rt / (1._rt + radial_numpercell_power));
                ...
                const amrex::Real flux =
                    inj_flux->getFlux(ppos.x, ppos.y, ppos.z, t);
                ...
                UpdatePosition(ppos.x, ppos.y, ppos.z, pu.x, pu.y, pu.z,
                               t_fract, mass);
        ...
        amrex::Gpu::synchronize(); // If this is removed, we need to make sure
                                   // inj_rho is async safe.
    ...
    // Redistribute the new particles that were added to the temporary
    // container. (This eliminates invalid particles, and makes sure that
    // particles are in the right tile.)
    tmp_pc.Redistribute();
